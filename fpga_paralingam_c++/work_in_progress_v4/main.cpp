...
// Creates the same sample data as the Python example.
Matrix get_matrix() {
    std::cout << "Creating sample matrix..." << std::endl;
...
// Read a CSV file into the Matrix struct.
Matrix read_csv(const std::string& filepath) {
    ...
    // This skips a header row if one exists.
    ...
    while (std::getline(file, line)) {
        ...
        // Column counter to skip the first column - index 0
        int col_idx = 0;
        while (std::getline(ss, cell, ',')) {
            if (col_idx > 0) { 
		// Only process columns after the first one.
                try {
...
int main(int argc, char* argv[]) {
    std::cout << "Running ParaLiNGAM Algorithm" << std::endl;
    ...
    try {
        if (argc < 2) {
            // If no CSV is provided, use the exmaple matrix.
            std::cout << "No CSV file provided. Falling back to sample." << std::endl;
            data = get_matrix();
        } else {
